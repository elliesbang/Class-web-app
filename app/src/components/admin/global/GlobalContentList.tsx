...
const GlobalContentList = ({ refreshToken, onEdit, onDeleted }: GlobalContentListProps) => {
  ...
  useEffect(() => {
    ...
    const fetchList = async () => {
      ...
      try {
        ...
        if (!isMounted) return;
        console.error('[content] list load failed (global)', caught);
  ...
  const handleDelete = async (itemId: number | string) => {
    ...
    try {
      const response = await fetch(`/api/admin-content-global-delete/${numericId}`, {
        method: 'DELETE',
      });
      if (!response.ok) {
            ...
            return (
              <li key={itemId} className="flex flex-col gap-3 py-3 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  ...
                  <p className="text-sm text-ellieGray/70">{item.content ?? ''}</p>
                </div>
