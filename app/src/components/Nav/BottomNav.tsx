...
const BottomNav = () => {
  ...
  return (
    <nav className="fixed inset-x-0 bottom-0 z-20 bg-[#fefaf4] px-5 pb-6 pt-3 shadow-[0_-6px_20px_rgba(0,0,0,0.05)]">
      <div className="grid gap-3" style={{ gridTemplateColumns: `repeat(${navItems.length}, minmax(0, 1fr))` }}>
        {navItems.map(({ label, to, Icon }) => {
          const isActive = to === '/' ? location.pathname === '/' || location.pathname === '/home' : location.pathname.startsWith(to);
