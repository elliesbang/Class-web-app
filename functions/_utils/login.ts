...
const passwordsMatch = (plain: string, stored: string): boolean => {
  ...
  if (stored.startsWith('$2')) {
    try {
      ...
      console.warn('[AUTH] Failed to compare bcrypt hash', error);
      return false;
    }
  }
  return stored === plain;
...
export const handleLoginRequest = async (
  ...
  const statement = env.DB.prepare(`SELECT * FROM ${table} WHERE email = ?1 LIMIT 1`).bind(email);
  ...
  if (password) {
    if (!storedHash || !passwordsMatch(password, storedHash)) {
      throw new ApiError(401, { error: 'Invalid credentials' });
    }
  } else if (name) {
    const recordName = extractName(record, null);
    if (recordName && recordName.localeCompare(name, 'ko', { sensitivity: 'base' }) !== 0) {
      throw new ApiError(401, { error: 'Invalid credentials' });
    }
  } else if (storedHash) {
    throw new ApiError(401, { error: 'Password required' });
  }

  const userId = stringOrNull(record.id) ?? stringOrNull(record.user_id) ?? stringOrNull(record.uuid);
  if (!userId) {
    throw new ApiError(500, { error: '계정 정보에 ID가 없습니다.' });
  }
